...
<body style="overflow-x: hidden">
    ...
    <div style="background-color: lightyellow;">
            ...
            　		<a href="/BandMoa/CommentModifyTry.do?rno=<%=cDTO.getRno() %>">관리</a>	
            <%} else { %>
            <a href="/BandMoa/CommentModifyTry.do?rno=<%=cDTO.getRno() %>">수정/삭제</a>
            <%} %>
            </div>
            </span>
    </div>
	...
	<form id="asdf" name="asdf" method="post" action="/DExellent/board/CommentProc.do?seq=<%=CmmUtil.nvl(bDTO.getBoard_seq()) %>">
	<div>
	...
	<% if (session.getAttribute("user_name").equals("조정규")) { %>
	<input type="button" onclick="location.href='/DExellent/board/BoardModifyCertifyTry.do?seq=<%=seq%>'" class="btn btn-primary btn-block" value="관리자 수정/삭제하기"><br>
	<%} else {%>
	<input type="button" onclick="location.href='/DExellent/board/BoardModifyCertifyTry.do?seq=<%=seq%>'" class="btn btn-primary btn-block" value="수정/삭제하기"><br>
	<%} %>
